...
/// Rebuild kuluu-mcp before spawning it. kuluu-mcp is a separate crate, so
/// `CARGO_BIN_EXE_*` is not populated for kuluu's tests; without this a
/// stale `target/debug/kuluu-mcp` (e.g. an old xiloader version string) is
/// silently spawned and rejected by the current LSB.
pub fn build_kuluu_mcp() -> Result<()> {
    let cargo = std::env::var("CARGO").unwrap_or_else(|_| env!("CARGO").to_string());
    let mut cmd = std::process::Command::new(cargo);
    cmd.args(["build", "-p", "kuluu-mcp"]);
    ...
    let status = cmd
        ...
        .status()
        .context("running `cargo build -p kuluu-mcp`")?;
    if !status.success() {
        return Err(anyhow!("`cargo build -p kuluu-mcp` failed: {status}"));
    }
    Ok(())
}

pub fn kuluu_mcp_bin() -> PathBuf {
...
impl McpClient {
    ...
    async fn read_one(&mut self) -> Result<Value> {
        ...
        if n == 0 {
            return Err(anyhow!("kuluu-mcp closed stdout"));
        }
