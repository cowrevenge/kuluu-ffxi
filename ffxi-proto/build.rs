...
fn write_login_settings(out_path: &std::path::Path) -> Result<()> {
    let src =
    ...
    let out = format!(
        "// AUTO-GENERATED by ffxi-proto/build.rs from {LSB_LOGIN_LUA}.\n\
         // Do not edit by hand.\n\
         pub const LSB_CLIENT_VER: &str = {client_ver:?};\n\
         pub const LSB_DEFAULT_VER_LOCK: u8 = {ver_lock};\n",
    );
    fs::write(out_path, &out)?;
    println!("ffxi-proto: scraped login.CLIENT_VER {client_ver} login.VER_LOCK {ver_lock}");
    Ok(())
}
...
/// `ALL_KNOWN` (every member not named UNUSED_*) for the C2S 0x26 excode the
/// client advertises and a `name` lookup for the error reply.
fn write_lobby_tables(path: &std::path::Path) -> Result<()> {
    ...
    for (src_path, enum_name, module) in [
        (
            LSB_LOGIN_HELPERS_H,
            "EXPANSION_DISPLAY",
            "expansion_display",
        ),
        (LSB_LOGIN_HELPERS_H, "FEATURE_DISPLAY", "feature_display"),
        (LSB_LOGIN_ERRORS_H, "errorCode", "lobby_error"),
    ] {
