...
use common::mcp_client::{build_kuluu_mcp, is_reachable, kuluu_mcp_bin, read_text, McpClient};
...
async fn agent_session_drives_mcp_end_to_end() {
    ...
    // kuluu-mcp is a separate crate, so CARGO_BIN_EXE_ isn't available here;
    // rebuild it so a stale target/debug/kuluu-mcp is never spawned.
    build_kuluu_mcp().expect("cargo build -p kuluu-mcp");
    let bin = kuluu_mcp_bin();
    assert!(
        bin.exists(),
        "kuluu-mcp binary not found at {} even after `cargo build -p kuluu-mcp`",
        bin.display()
    );
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                tracing_subscriber::EnvFilter::new("info,kuluu=debug,kuluu_mcp=debug")
            }),
        )
        .with_test_writer()
        .try_init();
    ...
    let goal_path = std::env::temp_dir().join(format!("kuluu-mcp-it-goal-{}.json", fixture.charid));
    ...
    let mut child = Command::new(&bin)
        ...
        .env("RUST_LOG", "info,kuluu=info,kuluu_mcp=info")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::inherit())
        .kill_on_drop(true)
        .spawn()
        .expect("spawn kuluu-mcp");
    ...
    match timeout(Duration::from_secs(5), child.wait()).await {
        Ok(Ok(status)) => eprintln!("kuluu-mcp exited: {status:?}"),
        Ok(Err(e)) => eprintln!("kuluu-mcp wait err: {e:#}"),
        Err(_) => {
            eprintln!("kuluu-mcp didn't exit in 5s after stdin close, killing");
