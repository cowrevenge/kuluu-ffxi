# FFXI / Kuluu — DLSS 5 Neural Uplift (DLSSNR) integration notes
...
**STATUS (build 13, compiled clean — AWAITING USER'S TEST via `.\dlssplay.bat`):** build 12's four fixes (§2.12) were in source and compiled; the user tested it and hit one new panic: wgpu-core 29 forbids mixing high-level + raw encoding on a single CommandEncoder (EncodingApi locks on first use — §2.9 item 11), which my MVec clear pass did against the create encoder. Fixed with two encoders (clear = high-level-only, submitted first; create = raw-only) and recompiled: build 13 = clean release build of all fixes. The exe is already built + synced; no rebuild needed unless source changes.
...
`shutdown()` documented as gated (no trampoline yet — nothing calls it). **Next: user runs `.\build-dlss.bat`
(NO arg — release + root-exe sync), then `.\dlssplay.bat`; expected log lines in §5, return-code table in §3.5.**

---

## 1. Current setup (what exists today)

### Runtime stack next to the exe (repo root + `target/debug/`)
| File | Role | Status |
|---|---|---|
...
| `nvngx_dlssnr.dll` (~166 MB) | **NR backend runtime v310.8.0** (NVIDIA-signed) | **staged at root + target/debug this session**; `build-dlss.bat` now auto-stages it when missing |

### Build/run tooling (all personal, git-excluded via `.git/info/exclude`)
- `build-dlss.bat` (repo root): release default, `debug` arg, `--jobs 8`, syncs exe via `logs\sync_exe.ps1`;
  **new this session**: stages `nvngx_dlssnr.dll` next to both exes when missing. **User builds; we only hand over the cmd.**
- `dlssplay.bat`: sets `WGPU_ADAPTER_NAME=NVIDIA`, `SL_LOG_PATH=%~dp0streamline\logs`. No env vars in code — batch files only.
- SL logs go to `streamline/logs/` (never `.verify/`).
- Excluded patterns: `/streamline/`, `/nvngx_*.dll`, `/sl.*.dll`, `/build-dlss.bat`, `/dlssplay.bat` (all verified via git check-ignore).
...
- Bevy 0.19 `bevy_anti_alias` → `dlss_wgpu` crate: SR via wgpu HAL raw Vulkan handles;
  ...
  (duplicate-plugin panic). Uncommitted fix in `kuluu/src/view_native/mod.rs` (~line 345: removed manual
  `app.add_plugins(DlssInitPlugin)`, kept only `insert_resource(project_id())`) + doc updates in
  `kuluu-render/src/graphics/dlss.rs`. **Still uncommitted — commit with or ahead of the NR work.**
...
- **NEW this session — the NR pipeline** (all behind `feature = "dlss"`, native-only):
  ...
  - Settings: Neural Uplift toggle + NR Intensity / Local Tone Strength / Structure Strength rows are now LIVE
    (were inert placeholders); DLSS removed from the AA cycler per user request (explicit on/off row only).
...
- `nvngx_dlssnr.dll` staged at repo root + target/debug; build-dlss.bat auto-stages when missing.
...
**Naming.** Cargo builds it as `kuluu_ngx_fwd.dll` (lib names can't contain `.`). `build-dlss.bat` copies it
...
| `0xBAD0_0002` | still module-gated on THAT call: forwarder missing/stale next to the exe (build-dlss.bat re-stages it every build) — applies to Init_Ext, CreateFeature AND ReleaseFeature (§2.10 scope map) |
...
- No dependency edge from kuluu-render or kuluu-dlss-nr; it is loaded at runtime like the NR DLL — so
  **build-dlss.bat builds it explicitly** (`cargo build -p kuluu-ngx-fwd`, same profile as the main build);
...
**build-dlss.bat additions** (after the existing nvngx_dlssnr.dll staging):
```bat
...
1. `.\build-dlss.bat` (no arg) → `kuluu_ngx_fwd.dll` appears in `target\release\`, staged copy
   `nvngx.dll_kuluu.dll` at repo root + target/release. `objdump -p nvngx.dll_kuluu.dll | grep kuluu_ngx_fwd`
   shows the three exports.
2. `.\dlssplay.bat` → log shows `dlss-nr: forwarder v1 loaded (nvngx.dll_kuluu.dll)` then
...
4. **Runtime diagnosis CLOSED (§2.9 item 8)**: GetProcAddress NULL was our own fault — `resolve!` passed an
   ...
   HDR input acceptance, extension requirements, first-frame ordering. Build 13 is already compiled + synced — just run `.\dlssplay.bat` (§5).
...
1. `.\dlssplay.bat`; check SL logs (`streamline/logs/`) for:
...
```
ffxi_dlss5.md                     ← this doc
build-dlss.bat / dlssplay.bat     personal launchers (repo root, excluded); build-dlss now stages nvngx_dlssnr.dll
.git/info/exclude                 personal exclusions (/streamline/, /nvngx_*.dll, /nvngx.dll_*.dll, /sl.*.dll, both .bat)
nvngx_dlssnr.dll                  staged at repo root + target/debug/ (excluded via /nvngx_*.dll)
nvngx.dll_kuluu.dll               build 9 forwarder staged copy (repo root + target/{debug,release}; excluded via /nvngx.dll_*.dll)
...
kuluu-ngx-fwd/                    NEW build 9: forwarder cdylib (zero deps; staged as nvngx.dll_kuluu.dll) — COMMIT THE CRATE
kuluu-dlss-nr/                    FFI crate (~700 lines incl. forwarder routing in load()/init()) — UNCOMMITTED
kuluu-render/src/graphics/dlss_nr.rs   NEW: pipeline hook (~490 lines) — UNCOMMITTED
kuluu-render/src/graphics/settings.rs  NR rows live; AA cycler cleaned up; nr_active() — UNCOMMITTED
kuluu-render/src/graphics/mod.rs       dlss_nr module decl — UNCOMMITTED
kuluu-render/Cargo.toml            optional kuluu-dlss-nr dep + feature wiring — UNCOMMITTED
kuluu-render/src/lib.rs            ViewerCorePlugin register() call — UNCOMMITTED
Cargo.toml                         workspace members/default-members — UNCOMMITTED
kuluu/src/view_native/mod.rs       duplicate-plugin fix (last session) — UNCOMMITTED
kuluu-render/src/graphics/dlss.rs  doc updates (last session) — UNCOMMITTED
