...
fn write_login_settings(out_path: &std::path::Path) -> Result<()> {
    ...
    let out = format!(
        "// AUTO-GENERATED by ffxi-proto/build.rs from {LSB_LOGIN_LUA} and {LSB_NETWORK_LUA}.\n\
         ...
         pub const LSB_LOGIN_AUTH_PORT: u16 = {auth_port};\n\
         pub const LSB_LOGIN_DATA_PORT: u16 = {data_port};\n\
         pub const LSB_LOGIN_VIEW_PORT: u16 = {view_port};\n",
    );
    fs::write(out_path, &out)?;
    println!(
        "ffxi-proto: scraped login.CLIENT_VER {client_ver} login.VER_LOCK {ver_lock} \
         network ports auth={auth_port} data={data_port} view={view_port}"
    );
    Ok(())
}
...
/// C2S 0x26 excode the client advertises, and a `name` lookup on each.
