...
struct MoonUniform {
    ...
    /// z = intensity multiplier, w unused (vec4 alignment).
    params: vec4<f32>,
...
// Retail's moon is a flat, luminous disc, so limb darkening only keeps the rim from reading
// as a hard cut, and the disc rides a touch brighter for it.
const LIMB_MIN: f32 = 0.96;
const FLAT_DISC_GAIN: f32 = 1.08;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let brightness = smoothstep(-0.02, 0.02, uv.x * waxing_sign - term_x);

    let limb = mix(LIMB_MIN, 1.0, sqrt(max(0.0, 1.0 - r2))) * FLAT_DISC_GAIN;
