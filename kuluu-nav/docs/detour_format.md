...
Reference for a future Rust-native parser. Today `kuluu-nav` only
...
pathfinding hand-trace PNGs into `~/.config/kuluu-mcp/heightmaps/`
in the meantime.
...
```rust
// kuluu-nav/src/detour.rs (future)
...
- `kuluu-nav/src/zone_names.rs` — id → name lookup
