...
async fn run_socket(
    ...
    let ws = match WebSocket::open(&url) {
        Ok(ws) => ws,
        Err(e) => {
            warn!("kuluu-viewer-wasm: WebSocket::open({url}) failed: {e:?}");
            return;
        }
    };
    info!("kuluu-viewer-wasm: connected to {url}");
            ...
            Ok(Message::Bytes(bytes)) => match postcard::from_bytes::<Frame>(&bytes) {
                Ok(Frame::Hello { protocol_version }) => {
                    if protocol_version != PROTOCOL_VERSION {
                        warn!(
                            "kuluu-viewer-wasm: protocol version mismatch \
                                 (server={protocol_version}, viewer={PROTOCOL_VERSION}); \
                                 continuing optimistically"
                        );
                    } else {
                        info!("kuluu-viewer-wasm: hello, protocol_version={protocol_version}");
                    }
                }
                ...
                Err(e) => {
                    warn!("kuluu-viewer-wasm: postcard decode error: {e:?}");
                }
            },
            Ok(Message::Text(t)) => {
                warn!(
                    "kuluu-viewer-wasm: unexpected text frame ({} bytes), ignoring",
                    t.len()
                );
            }
            Err(e) => {
                warn!("kuluu-viewer-wasm: websocket recv error: {e:?}");
                break;
            }
        }
    }
    ...
    info!("kuluu-viewer-wasm: socket closed");
}
