...
use ffxi_viewer_core::hud::auction::{
...
use ffxi_viewer_core::hud::digit_spinner::DigitSpinner;
...
pub fn auction_mode_sync_system(
    ...
    events: Res<ffxi_viewer_core::EventLog>,
    mut cursor: Local<u64>,
    mut mode: ResMut<InputMode>,
    mut screen: ResMut<ffxi_viewer_core::hud::auction::AuctionScreenState>,
) {
    ...
    for i in (*cursor).max(first_global)..total {
        if matches!(
            ...
            ffxi_viewer_wire::ViewerEvent::AuctionMenuOpened
        ) {
            opened = true;
        }
    }
...
fn nav_horizontal(
    ...
    use ffxi_viewer_core::hud::menu::page_cursor;
    let snap = &scene_state.snapshot;
    let catalog_len = auction::filtered_listings(screen, snap).len();
    match &mut screen.screen {
        AhScreen::Catalog {
    ...
    match screen.screen.clone() {
        ...
        AhScreen::Catalog {
            ...
            match menu_cursor {
                ...
                1 => {
                    let gil = ffxi_viewer_core::hud::delivery::current_gil(&scene_state.snapshot);
...
mod tests {
    use super::*;
    use ffxi_viewer_core::hud::auction::SellRow;
    use tokio::sync::mpsc;
    ...
    fn sell_stack_flow_quotes_confirms_and_records_the_echo() {
        ...
        scene.snapshot.auction.fee_quote = Some(ffxi_viewer_wire::AhFeeQuote {
            fee: 9,
            inventory_slot: 1,
            item_no: 4570,
            stack: true,
