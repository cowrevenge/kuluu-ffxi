...
fn print_help() {
    eprintln!(
        r#"kuluu-mcp-repl — line-oriented MCP driver
...
async fn main() -> Result<()> {
    ...
    if !bin.exists() {
        return Err(anyhow!(
            "kuluu-mcp binary not found at {}. Build first: `cargo build -p kuluu-mcp`",
            bin.display()
        ));
    }

    eprintln!("kuluu-mcp-repl: spawning {}", bin.display());
    ...
    let init = mcp
        .request(
            ...
            json!({
                ...
                "capabilities": {},
                "clientInfo": { "name": "kuluu-mcp-repl", "version": "0.1.0" },
            }),
        )
        .await
        .context("initialize")?;
    eprintln!(
        "kuluu-mcp-repl: connected to {}",
        init.get("serverInfo")
    ...
    eprintln!("kuluu-mcp-repl: ready. type `help` for usage.");
    ...
    eprintln!("kuluu-mcp-repl: shutting down");
