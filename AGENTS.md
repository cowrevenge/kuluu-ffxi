...
- **State contracts cannot self-skip.** `scripts/checks.sh contracts` runs the mandatory event contract without game assets or servers and fails if the test is missing. CI's test stage, pre-push (including fast mode), and pre-commit for event/protocol/session changes run it. Event VM numeric displays derive from its work slots. Production dialog steps require the transport module's private permit and return a sealed prepared packet; the outcome is exposed only after transmission succeeds. Extend the synthetic raw-packet contract when changing event state, acknowledgements, cancellation, or packet ordering. Verify new assertions with a deliberate failing mutation before trusting them.
