...
use common::mcp_client::{is_reachable, kuluu_mcp_bin, read_text, McpClient};
...
async fn disconnect_recovery_reconnects_after_map_restart() {
    ...
    if !bin.exists() {
        panic!(
            "kuluu-mcp binary not found at {}.\n\
             Build it first: `cargo build -p kuluu-mcp`",
            bin.display()
        );
    }
    ...
    let goal_path = std::env::temp_dir().join(format!("kuluu-mcp-recover-{}.json", fixture.charid));
