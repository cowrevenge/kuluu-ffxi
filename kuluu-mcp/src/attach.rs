...
use kuluu_session::agent_socket::AGENT_PIDFILE_NAME;
use kuluu_session::state::{AgentCommand, AgentEvent};
...
fn resolve_pidfile(pidfile: &Path) -> Result<PathBuf> {
    let body = std::fs::read_to_string(pidfile).with_context(|| {
        format!(
            "reading agent pidfile {} (was kuluu started with `--agent-listen auto`?)",
            pidfile.display()
        )
    })?;
...
pub async fn run(
    ...
    loop {
        match UnixStream::connect(&sock).await {
            Ok(stream) => {
                eprintln!("kuluu-mcp attached to agent socket at {}", sock.display());
                tracing::info!(path = %sock.display(), "attached to kuluu agent socket");
...
mod tests {
    use super::*;
    use kuluu_session::agent_socket;
    use std::time::Duration;

    fn temp_sock(label: &str) -> PathBuf {
        let mut p = std::env::temp_dir();
        p.push(format!(
            "kuluu-mcp-attach-{}-{}.sock",
            label,
            std::process::id()
        ));
