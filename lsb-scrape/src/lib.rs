...
/// running, so the header names the actual generator.
fn generated_header(source_path: &str) -> String {
    let pkg = std::env::var("CARGO_PKG_NAME").unwrap_or_else(|_| "unknown".into());
    format!("// AUTO-GENERATED by {pkg}/build.rs from {source_path}.\n")
}
